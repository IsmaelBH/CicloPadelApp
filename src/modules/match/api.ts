...
export async function createMatchAtomic(params: {
    date: string; courtId: Court['id']; start: string; duration: 90 | 120;
    category: Category; creator: { uid: string; name: string; photoUrl?: string | null }
}) {
    ...
    return runTransaction(db, async (tx) => {
        ...
        const match: MatchDoc = {
            ...
            status: 'en_formacion',
            players: [{
                uid: creator.uid, name: creator.name, photoUrl: creator.photoUrl ?? null,
...
export async function joinMatchAtomic(params: {
    ...
    return runTransaction(db, async (tx) => {
        ...
        const already = players.some(p => p.uid === user.uid);
        if (already) return { ok: true, matchId, status: m.status, already: true as const };

        players.push({
            uid: user.uid, name: user.name, photoUrl: user.photoUrl ?? null,
            joinedAt: Math.floor(Date.now() / 1000)
        });

        const status = players.length === 4 ? 'completa' : 'en_formacion';
        tx.update(mRef, { players, status });
        return { ok: true, matchId, status, already: false as const };
    });
}
